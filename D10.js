...
console.log(deleteOne("Renato", true));
...
let onlyLetter = function (string) {
...
console.log(isThisAnEmail("emaildiprova@hotmail.it"));
console.log(isThisAnEmail("emaildiprova@it"));
...
const rollTheDices = function (numb) {
  let sum = 0;
  let values = [];
...
const howManyDays = function (data) {
  ...
  const diff = Math.abs(present - past);
  const totalDays = Math.floor(diff / (1000 * 60 * 60 * 24));
  return totalDays;
};
console.log(howManyDays("2004-12-4"));
...
const isTodayMyBirthday = function () {
  const oggi = new Date();
  const myDate = new Date("2004-12-4");
  if (oggi.getDate() === myDate.getDate()) {
    return true;
  }
  return false;
};
...
const newestMovie = function (movies) {
  ...
  for (let i = 1; i < movies.length; i++) {
    if (parseInt(movies[i].Year) > parseInt(ultimo.Year)) {
      ultimo = movies[i];
    }
  }
  return ultimo;
